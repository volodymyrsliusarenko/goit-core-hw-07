...
class Record:
    ...
    def add_birthday(self, birthday):
        try:
            self.birthday = Birthday(birthday)  # Перевіряємо та зберігаємо день народження
    ...
    def get_upcoming_birthdays(self, days=7):
        ...
        for record in self.data.values():
            if record.birthday:
                ...
                # Переносимо на наступний рік, якщо день народження вже пройшов у поточному році
                if birthday_this_year < today:
                    birthday_this_year = datetime(today.year + 1, record.birthday.date.month, record.birthday.date.day)
    
                delta = birthday_this_year - today
    
                if 0 <= delta.days <= days: # Перевіряємо лише майбутні 7 днів
                    ...
                    upcoming_birthdays.append({"name": record.name.value, "birthday": birthday_this_year.strftime("%d.%m.%Y")})  # Додаємо дату у форматі ДД.ММ.РРРР
...
def change_contact(args, book: AddressBook):
    ...
    record = book.find(name)
    if record:
        record.edit_phone(record.phones[0].value, new_phone)  # Припускаємо, що в кожного контакту є лише один номер телефону
